...
int array_partition(int *ar,int left,int right,int pivot)
{
    int leftptr=left-1;
    int rightptr=right;
    while(true)
...
void rec_quick_sort(int *ar,int left,int right)
{
    ...
    {
        int pivot= ar[right];
        int part=array_partition(ar,left,right,pivot);
...
void rec_rand_quick_sor(int *ar,int left,int right)
{
    if(right-left<=0)
    {
        return;
    ...
    {
        int pivot= ar[left+rand()%(right-left)];
        int part=array_partition(ar,left,right,pivot);
        rec_quick_sort(ar,left,part-1);
        rec_quick_sort(ar,part+1,right);
    }
